(function () {
  ...
  var groundPos = 210;
  var gameSession = 0;    //遊戲階段
  var stageFlag = 500;     //每個階段的分差
  ...
  Runner.config = {
    ...
    BOTTOM_PAD: 60,                        // 小恐龙距 canvas 底部的距离
  ...
  Runner.spriteDefinition = {
    LDPI: {
      HORIZON: { x: 0, y: 0 },     // 地面
      CLOUD: {x: 528, y: 304},
      KAGEYAMA: {x: 2, y: 300}, // 影山
      USHIJIMA: {x: 102, y: 300}, // 牛島若利
      PTERODACTYL: {x: 134, y: 42},  // 翼龙
      TEXT_SPRITE: {x: 528, y: 480},  // 文字
      RESTART: {x: 720, y: 480},        // 重置游戏按钮

      TREX: {x: 190, y: 300},         // oikawa
    },
  };
  ...
  GameOverPanel.dimensions = {
    TEXT_X: 528,          // 文字 "Game Over" 的 x 坐标
    TEXT_Y: 500,
    TEXT_WIDTH: 184,    // 文字 "Game Over" 的宽度
    TEXT_HEIGHT: 22,
    RESTART_WIDTH: 40,  // 重置按钮的宽度
    RESTART_HEIGHT: 40,
  };
  ...
  GameOverPanel.prototype = {
    draw: function() {
      ...
      var textTargetY = Math.round((this.canvasDimensions.HEIGHT) / 2 -30);
...
// 生成背景物件
  ...
  function HorizonLine(canvas, spritePos) {
    ...
    this.dimensions = {};       // sprite size
    this.spritePos = spritePos; // sprite在src位置
    this.sourceXPos = [];       // 兩種地形在src的x座標
    this.xPos = [];             // canvas 中地面的 x 
  ...
  HorizonLine.prototype = {
    init: function () {
      ...
      //sprite在src上的兩個x座標 (現在都抓同一個所以同一個參數)
      this.xPos = [0, 0];
    ...
    draw: function () {
      ...
      // 兩個地形
      this.ctx.drawImage(
        Runner.imageSprite,                   // src image
        this.sourceXPos[0], this.spritePos.y, // sprite pos on src image
        this.dimensions.WIDTH, this.dimensions.HEIGHT, //畫出大小
        this.xPos[0], this.yPos,              // 畫在canvas 
        this.dimensions.WIDTH, this.dimensions.HEIGHT, //canvas畫出大小
      );
      this.ctx.drawImage(
        Runner.imageSprite,
    ...
    getRandomType: function () {
      return Math.random() > this.bumpThreshold ? 0 : 0;
      //0,0 -> spriteXPos value
    },
    reset: function() {
      this.xPos[0] = 0;
      this.xPos[1] = 0;
  ...
  Cloud.config = {
    WIDTH: 128 ,
    HEIGHT: 34,
    MIN_CLOUD_GAP: 300,   // 云之间的最小间隙
    MAX_CLOUD_GAP: 500,   // 云之间的最大间隙
    
    BG_CLOUD_SPEED: 0.2,  // 云的速度
    CLOUD_FREQUENCY: 0.1, // 云的频率
    MAX_CLOUDS: 4 ,         // 云的最大数量
  ...
  Cloud.prototype = {
    ...
    draw: function () {
      ...
      this.spritePos.x = 528 + Cloud.config.WIDTH * this.flag[0]
      this.spritePos.y = 304 + Cloud.config.HEIGHT * this.flag[1]
      ...
      this.ctx.drawImage(
        ...
        this.xPos, this.yPos,
        outputWidth*1.5, outputHeight*1.5
    ...
    isVisible: function () {
      ...
      return this.xPos + Cloud.config.WIDTH*1.5 > 0;
    },

    getRandomFlag: function () {
      let xMax = 2
      let yMax = 2
      if (gameSession == 2) yMax = 5 
      let xIndex = Math.floor(Math.random() * xMax);
      let yIndex = Math.floor(Math.random() * yMax);
      let flagIndex = [xIndex, yIndex]
      return flagIndex;
    },
  };
  ...
  Obstacle.types = [{
    ...
    width: 46,
    height: 64,
    yPos: groundPos,             // 在 canvas 上的 y 坐标
    multipleSpeed: 4,
    minGap: 200,           // 最小间距
    minSpeed: 0,           // 最低速度
    numFrames: 2,          // 两个动画帧  
    frameRate: 1000 / 2,   // 帧率（一帧的时间）
    collisionBoxes: [      // 碰撞盒子
      new CollisionBox(12, 0, 26, 4),
      new CollisionBox(6, 6, 38, 6),
      new CollisionBox(4, 12, 42, 16),
      new CollisionBox(6, 28, 38, 37),
    ],
  }, {
    type: 'USHIJIMA',  // 牛島若利
    width: 42,
    ...
    frameRate: 1000 / 2,   // 帧率（一帧的时间）
    collisionBoxes: [      // 碰撞盒子
      new CollisionBox(8, 0, 26, 6),
      new CollisionBox(0, 7, 42, 26),
      new CollisionBox(5, 33, 34, 34),
    ],
  }, 
  // {
  //   type: 'PTERODACTYL',   // 翼龙
  ...
  Obstacle.prototype = {
    init: function (speed) {
      ...
      //隨遊戲階段改變sprite
      this.spritePos.y +=  gameSession * this.spriteOffset;
  ...
  function Trex(canvas, spritePos) {
    ...
    this.xPos = 0;
    this.yPos = 0;
    this.groundYPos = 100;               // 小恐龙在地面上时的 y 坐标
  ...
  Trex.config = {
    ...
    WIDTH: 50,                  // 站立时的宽度
    HEIGHT: 66,
    MAX_JUMP_HEIGHT: 32,        // 最大跳跃高度
    MIN_JUMP_HEIGHT: 32,        // 最小跳跃高度
    SPRITE_WIDTH: 300,          // 站立的小恐龙在雪碧图中的总宽度
    DROP_VELOCITY: -7,          // 下落的速度
  ...
  Trex.BLINK_TIMING = 3000;     // 眨眼最大间隔的时间

  // 小恐龙的碰撞盒子
  Trex.collisionBoxes = {
    RUNNING: [
      new CollisionBox(14, 0, 28, 8),
      new CollisionBox(10, 8, 34, 4),
      new CollisionBox(4, 12, 46, 13),
      new CollisionBox(6, 25, 42, 33),
      new CollisionBox(18, 58, 8, 9),
      new CollisionBox(30, 58, 8, 9)
    ]
  };
  ...
  Trex.prototype = {
    ...
    draw: function(x, y) {
      ...
      sourceY += gameSession * this.spriteOffset
    ...
    update: function (deltaTime, distance) {
      ...
      if (!this.achievement) {
        ...
        if (distance > 0) {
          ...
          //控制遊戲階段
          if(sessionFlag && gameSession != 2){
            if (!(distance % stageFlag)){
              if(!this.stageCount){
                this.stageCount = true;
                gameSession += 1;
                if (gameSession > 2 ) gameSession = 2;
              }
              // console.log(this.stageCount, distance, gameSession)
  ...
  Horizon.prototype = {
    ...
    addNewObstacle: function(currentSpeed) {
      ...
      if (this.duplicateObstacleCheck(obstacleType.type) ||
        ...
        var obstacleSpritePos = new Object()
        switch(obstacleType.type){
          case "KAGEYAMA":
            obstacleSpritePos.x = 2;
            obstacleSpritePos.y = 300;
            break;
          case "USHIJIMA":
            obstacleSpritePos.x = 102;
            obstacleSpritePos.y = 300;
            break;    
        }
        console.log(obstacleType.type, obstacleSpritePos)
